  ...
  public static function setDefaultValue(array &$elements, array $value): void {
    foreach (Element::children($elements) as $child) {
      ...
      if (\is_array($default_value)) {
        if (isset($elements[$child]) && is_array($elements[$child])) {
          $child_count = count(Element::children($elements[$child]));
          $type = $elements[$child]['#type'] ?? NULL;
          if (empty($child_count) || $type === 'custom_field_multivalue') {
            $elements[$child]['#default_value'] = $default_value;
          }
          else {
            static::setDefaultValue($elements[$child], $default_value);
          }
        }
      }
      else {
